...
Three Reasoning Engines carry the managed evidence — the original two proofs,
each demonstrated in isolation, and the engine that converged them:
...
| The live run operates unattended, and the clock is load-bearing | 153 events on the durable trace of run `relief-live-1`: ~20 scheduler-driven observations against real weather (`PLAN_REBOUND` + `COMMIT_REAFFIRMED` each cycle, redeliveries absorbed), one hazard-triggered revocation at the start, and — at seq 135 — the **natural clock-triggered revocation**: `COMMIT_REVOKED` *"departure 12:00 has passed (now 12:00)"*, fired by the scheduler at 12:00 local with nobody watching, re-committed as `relief-r21-p4`. The trace spans the run's own development history — early events carry the pre-2026-08-31 plan-id scheme — which is evidence of a durable run surviving code evolution, not a single-version artifact | [`geap/relief_live_1_trace_snapshot.json`](geap/relief_live_1_trace_snapshot.json) *(captured 2026-08-31 after seq 135; the run is live and keeps growing)* | COMMITTED + LIVE |
...
| Agent Registry | eight registered services live — the original `harbor-weather` and `harbor-cargo-ops`, plus the six actor-path destinations registered denial-first during the 2026-08-31 convergence. Registration decides whether a destination is *known*; it is **not** an authorization allowlist — egress is authorized by a per-endpoint IAP IAM grant, which is why a registered destination with no `iap.egressor` binding is still denied | [`geap/iap_endpoint_policies.json`](geap/iap_endpoint_policies.json) *(the original two endpoints' IAP policies)*; live `agentregistry.googleapis.com` | COMMITTED + LIVE |
...
| The submitted tree | Engineering SHA `687eebfd26f64d87f3c8db49756f838dc90bc02a` is the original frozen code/evidence baseline, **not** the final submitted tree. The frozen core remained unchanged while later layers were added and explicitly recorded: `app/geap/` and its evidence, the sentinel (`app/sentinel.py`, `tests/test_sentinel.py`, engineering SHA `4296831`), ReliefRun (`app/scenarios/reliefrun.py`, `app/relief_demo.py`, `tests/test_reliefrun.py`, engineering SHA `893f759`), the portal (`app/portal.py`, `app/static/relief.html`, `tests/test_portal.py`, engineering SHA `9803336`), ReliefFleet with metrics and the seismic stream (`app/scenarios/relieffleet.py`, `app/fleet_demo.py`, `app/metrics.py`, `app/providers/seismic.py`, their tests), and the convergence/evidence artifacts (`geap/d2_*`, `geap/gw_logs_converged*`). Those additive layers have their own later revisions, including the 2026-08-31 clock semantics, portal lifecycle and USGS lookback fixes. "Unchanged" therefore describes the frozen core, not the entire submitted tree | [`docs/CORE_FREEZE.md`](docs/CORE_FREEZE.md) | STATED |
...
```bash
.venv/bin/python -m pytest tests -q                    # 315 passed, 47 skipped
