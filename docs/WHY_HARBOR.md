...
| **identity / policy boundary** | a managed actor runs under a Google-issued Agent Identity, and what it may reach is decided by IAM and the Gateway rather than by the code it is running. Demonstrated for one actor on the managed path, not for all three — see [`EVIDENCE.md`](../EVIDENCE.md) |
