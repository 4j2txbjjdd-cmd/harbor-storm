...
|---|---|
| Agent Registry | eight registered services returned live by `agentregistry.googleapis.com` — the original `harbor-weather` and `harbor-cargo-ops`, plus the six actor-path destinations registered denial-first during the 2026-08-31 convergence. The sharper claim: **registration is discovery, not authorization** — `harbor-cargo-ops` is registered and still denied, because per-endpoint IAP IAM authorizes, not the catalog |
| Agent Runtime | a real Gemini 3.5 Flash **executes inside it** against the five-tool bounded surface — exercised, not merely provisioned. The track defines Runtime as "long-running async execution". The two halves are demonstrated on separate runtime surfaces: Gemini executes inside Agent Runtime, while the unattended wall-clock revocation loop runs in Cloud Run via Cloud Scheduler (README §3). Since 2026-08-30 that loop has operated in the cloud against live state, including the seq-135 clock-triggered revocation. They are not combined on one managed runtime, and the deployed loop does not run a model actor |
